...
{
    (void)canAssign;
    char *text = (char *)malloc((size_t)parser.previous.length + 1);
    if (text == NULL)
    {
        error("Out of memory.");
        return;
    ...
    memcpy(text, parser.previous.start, (size_t)parser.previous.length);
    text[parser.previous.length] = '\0';
...
static void function(FunctionType type)
{
    ...
    ObjFunction *compiledFunction = endCompiler();
    emitBytes(OP_CONSTANT, makeConstant(OBJ_VAL(compiledFunction)));
}
...
bool compile(const char *source, ObjFunction **function)
{
